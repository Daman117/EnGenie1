...
const PdfSearchAndUpload = () => {
  ...
  // Get parent path by removing /upload from current path
  const getParentPath = () => {
    const path = location.pathname;
    if (path.endsWith('/upload')) {
      // Use slice to remove exactly '/upload' (7 characters) from the end
      const parentPath = path.slice(0, -7);
      return parentPath || '/solution';
    }
      ...
      <div className="w-full max-w-3xl transition-all duration-300 hover:scale-[1.02]">
        <div className="acrylic-glass-pill backdrop-blur-3xl p-8 relative transition-all duration-300 hover:shadow-2xl">
          ...
          <div className="text-center mb-6 space-y-4">
            ...
            <button
              onClick={() => navigate(getParentPath(), { replace: true })}
